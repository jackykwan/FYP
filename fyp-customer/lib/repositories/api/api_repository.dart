...
import 'package:customer_fyp/config/config.dart';
...
class ApiRepositoryImpl implements ApiRepository {
  ...
  Future<ApiResponse<Transaction>> getTransaction(
      {required String id, required String shopId}) async {
    try {
      final response = await api.post('/users/transaction', data: {
        'shopId': shopId,
        'transactionId': id,
      });
  ...
  @override
  Future<ApiResponse<User>> payTransaction(
    ...
    try {
      ...
      final result =
          await api.post('/users/transaction/pay', data: jsonEncode(body));
      return ApiResponse(result: User.fromJson(result.data));
    } on DioError catch (e) {
      final data = e.response?.data;
      if (data != null) {
  ...
  @override
  Future<ApiResponse<User>> topUp({required double amount}) async {
    try {
      final body = {
        'amount': amount,
      };
      final result = await api.post('/users/topup', data: jsonEncode(body));
      return ApiResponse(result: User.fromJson(result.data));
    } on DioError catch (e) {
      final data = e.response?.data;
      if (data != null) {
